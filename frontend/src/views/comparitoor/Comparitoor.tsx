...
export const Comparitoor = () => {
  ...
  const [pageData, setPageData] = useState<comparitoor.ComparitoorPage | null>(
    null,
  );
  ...
  const fetchData = useCallback(async () => {
    clearError();
    try {
      const result = await GetComparitoorPage(
  ...
  const currentData = useMemo(() => {
    ...
    switch (facet) {
      ...
      case types.DataFacet.ETHERSCAN:
        return pageData.transaction || [];
      case types.DataFacet.COVALENT:
        return pageData.transaction || [];
      case types.DataFacet.ALCHEMY:
        return pageData.transaction || [];
      default:
        LogError('[Comparitoor] unexpected facet=' + String(facet));
        return [];
    }
